...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        // mysql_connect($this->localhost,$this->user,$this->password) or die(mysql_error());
        $this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME'))."/".$_SESSION["user"].'/img/',
            'upload_url' => $this->get_full_url()."/".$_SESSION["user"].'/img/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $imgPath = "upload/".$_SESSION["user"]."/file/".$file->name;
                    // echo $imgPath;
                    mysql_connect($this->host,$this->user,$this->pass) or die(mysql_error());
                    mysql_select_db($this->db) or die(mysql_error());
                    $query = mysql_query("
                        INSERT INTO img(imgname,imgpath,create_by) 
                        VALUES('".$file->name."','".$imgPath."','".$_SESSION["user"]."')
                        ") 
                    or die(mysql_error());
                ...
                foreach($this->options['image_versions'] as $version => $options) {
                    if (!empty($version)) {
                        ...
                        if (is_file($file)) {
                            mysql_connect($this->host,$this->user,$this->pass) or die(mysql_error());
                            mysql_select_db($this->db) or die(mysql_error());
                            $queryDelete = mysql_query("DELETE FROM img
                                        WHERE imgname='".$file_name."' ") or die(mysql_error());
                            unlink($file);
                        }
                    }
                }
